...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, opt):
        ...
        self.layer2 = self._make_layer(
            block, 128, layers[1], shortcut_type, stride=down_stride_2)
        self.layer3 = self._make_layer(
            block, 256, layers[2], shortcut_type, stride=down_stride_2)
        self.layer4 = self._make_layer(
            block, 512, layers[3], shortcut_type, stride=down_stride_1)
        
        last_duration = int(1)
        last_size = int(math.ceil(sample_size / 32.0))
        self.maxpool_final = nn.MaxPool3d(
